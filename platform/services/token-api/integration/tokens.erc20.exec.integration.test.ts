...
import { amountMock2 } from '../test/mocks'
import {
...
describe('ERC20 - Exec', () => {
  ...
  beforeAll(async () => {
    scenario = new TestScenario('ERC20 - Exec')
    await scenario.init()
  ...
  describe('Kafka Command', () => {
    const commandPayload = () => {
      ...
      const params = [scenario.address, amountMock2]
  ...
  describe('HTTP Endpoint', () => {
    const requestArgs = () => ({
      functionName: 'mint',
      params: [scenario.address, amountMock2],
    })
    ...
    it('Throws when the token function does not exist', async () => {
      ...
      try {
        ...
        expect(error.response.data.message).toEqual(
          `tokenType=${TokenType.ERC20} functionName=${functionName} is not a function`,
        )
      }
    })
